...
Source lives under `src/`. Compiled HTML goes to `dist/` (generated by [Build commands](#build-commands); it is not committed to git).
...
| `src/blocks/` | Small reusable MJML fragments (for example a column, row or other repeatable layout). Not built on their own — only included inside other `.mjml` files. |
| `src/components/` | Full MJML documents (each has an `<mjml>` root). Built twice: HTML snippets for use in other `.mjml` files under `dist/components/` and full pages under `dist/components-raw/` (for independent use or for debugging build issues). |
